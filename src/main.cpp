...
const int microStepMode = 16;
const float pulleyDiameter = 30;
const int stepPerRev = fullSteps * microStepMode;
float stepPerMilimetre = stepPerRev / (3.141 * pulleyDiameter);
...
int incPos = 0;
float speed = 3.00;  //mm per second
  ...
  if (encDir != 0) {
    if (editing) {
      if (optionPos == 0) {
        ...
        stepPerMilimetre += encDir * increments[incPos];  // Adjust the steps per millimeter used for speed calcs
...
void changeSpeed() {
  ...
  if (encDir != 0) {
    ...
    speed += encDir * increments[incPos];
...
// Starts and stops the motor with the start/stop button
void startStop() {
  if (startButton.pressed()) {
